...
/*
On a hill as described in the statement, a point having a bigger x-coordinate
than another, will have a bigger z-coordinate as well. For each segment,
calculate the valid range of rotations in both directions, clockwise and
counter-clockwise, then calculate the intersection of all these ranges. The
answer is the smallest angle in the resulting ranges, if valid. An angle of zero
means the proposal is acceptable as is. A hill with an inclination of zero
degrees is a special case: all points have the same z-coordinate.
*/
